...
function createTimelineIndex (docIndex) {
  let timelineDefaults = {};

  // for teams we record events of when the team started or ended
  timelineDefaults.team = ['startedAt', 'endedAt'];

  // for team memberships we record events of when the membership started or ended
  timelineDefaults.teamMembership = ['startedAt', 'endedAt'];

  return require('./timeline-index')(docIndex, timelineDefaults);
}

// ----

function showTeamAtTime (docIndex, timelineIndex, teamId, givenTs, cb) {
