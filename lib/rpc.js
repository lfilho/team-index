...
module.exports = function (db, indexRegistry) {
  ...
  return {
    ...
    teams: {
      ...
      getMembershipsByPerson: function (personId, cb) {
        const personDoc = docIndex.getDoc(personId);
        if (!personDoc || personDoc._type !== 'person') {
          cb(new Error('not a valid person doc'));
        }
        ...
        teamIndex.through = function (entry) {
          ...
          // only index docs for the requested person
          if (doc.person !== personId) { return; }
        ...
        teamIndex.catchup();
        cb(null, {
          memberships: arrayFrom(teamIndex.memberships).map(bind(docIndex, 'getDoc'))
        });
      }
    },
